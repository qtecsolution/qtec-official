...
from dashboard.models import LegalTeamMembers, MarketingTeamMembers, TeamMembers
...
class MarketingMembersView(View):
    def get(self, request, id = None):
        if request.resolver_match.url_name == "save_marketing_member_url":
            return render(request, 'marketing_members/team_members_create.html')
        if request.resolver_match.url_name == "update_marketing_member_url":
            member = MarketingTeamMembers.objects.filter(id=id).first()
            context = {
                "member" : member,
            }
            return render(request, 'marketing_members/team_member_update.html', context)
        else:
            team_members = MarketingTeamMembers.objects.order_by('priority').values()
            context = {
                "team_members" : team_members,
            }
            return render(request, 'marketing_members/team_members.html', context)

    def post(self, request, id=None):
        data = request.POST
        file = request.FILES
        if request.resolver_match.url_name == "delete_marketing_member_url":
            request_id = request.POST.get('id')
            MarketingTeamMembers.objects.filter(id=request_id).first().delete()
            messages.success(request, 'Data deteted successful!')
            return redirect('dashboard:marketing_members_url')
